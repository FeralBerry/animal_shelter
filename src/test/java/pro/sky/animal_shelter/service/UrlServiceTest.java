...
class UrlServiceTest {
    ...
    UrlService urlService = new UrlService(adminService,
            userStatusService,
            startService,
            infoService,
            aboutService,
    ...
    void defineACommand() {
        ...
        doReturn(sendMessageList)
                .when(urlService)
                .defineACommand(update);
    }

    @Test
    void info() {
        setUpdate();
        InfoService infoServiceMock = Mockito.mock(InfoService.class);
        UrlService urlService = Mockito.mock(UrlService.class);
        doReturn(infoServiceMock.info())
                .when(urlService)
                .info(update);
        assertEquals(infoServiceMock.info(),urlService.info(update));
    }

    @Test
    void about() {
        setUpdate();
        AboutService aboutServiceMock = Mockito.mock(AboutService.class);
        UrlService urlServiceMock = Mockito.mock(UrlService.class);
        doReturn(aboutServiceMock.about())
                .when(urlServiceMock)
                .info(update);
        assertEquals(aboutServiceMock.about(),urlServiceMock.info(update));
    }
    ...
    void petReportForm() {
        ...
        UrlService urlServiceMock = Mockito.mock(UrlService.class);
        doReturn(petServiceMock.getPetForm())
                .when(urlServiceMock)
                .info(update);
        assertEquals(petServiceMock.getPetForm(),urlServiceMock.petReportForm(update));
    }

    @Test
    void contactInformation() {
        setUpdate();
        ContactInformationService contactInformationService = Mockito.mock(ContactInformationService.class);
        UrlService urlServiceMock = Mockito.mock(UrlService.class);
        doReturn(contactInformationService.getContactInformation())
                .when(urlServiceMock)
                .info(update);
        assertEquals(contactInformationService.getContactInformation(),urlServiceMock.petReportForm(update));
    }
    ...
    void petList() {
        ...
        PetService petService = Mockito.mock(PetService.class);
        UrlService urlServiceMock = Mockito.mock(UrlService.class);
        doReturn(petService.getPet(update))
                .when(urlServiceMock)
                .info(update);
        assertEquals(petService.getPet(update),urlServiceMock.petList(update));
    }
    ...
    void callVolunteer() {
        ...
        assertEquals(1L,urlServiceMock.callVolunteer(update));
    }
